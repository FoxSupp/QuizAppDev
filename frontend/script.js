...
const socket = io('http://192.168.178.212:3000');
...
if (isAdminPage) {
    ...
    if (!isAdmin) {
        ...
        function updatePressesList(presses) {
            pressesList.innerHTML = presses.map(press =>
                `<li>${press.username} (+${press.timeDiff}s)</li>`
            ).join('');
        }
        ...
        // Handle buzzer reset state
        socket.on('buzzersReset', () => {
            document.getElementById('winnerText').textContent = 'Warten auf Buzzer-Druck...';
        ...
        // Handle chat state updates
        socket.on('chatStateUpdate', (enabled) => {
            toggleChatButton.textContent = enabled ? 'Chat deaktivieren' : 'Chat aktivieren';
            toggleChatButton.classList.toggle('enabled', enabled);
            toggleChatButton.classList.toggle('disabled', !enabled);
        });

        // Handle buzzer state updates
        socket.on('buzzerState', ({ buzzerLocked, firstPresser, subsequentPresses, connectedUsers, userMessages: messages, chatEnabled }) => {
            if (buzzerLocked) {
